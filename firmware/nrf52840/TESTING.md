...
| Model contract or quantization drift | `model_quantization`, `model_runtime` | Exact input/output quantization, saturation, known partial INT8 reference values, artifact schema/operator/tensor metadata, static allocation, and a real TFLM invocation |
...
No complete reviewed 2,560-value model reference input is checked into this
repository. The model test therefore does not invent the missing 2,550 ECG
values from the supplied partial reference. It verifies all supplied INT8 RR
values and the first ten supplied ECG values by exact quantization round trip,
then runs the real embedded model on a deterministic zero-input vector and
compares its exact output, class, and confidence with an independent LiteRT
2.2.0 oracle. Add the full approved vector and its
expected `[0.0, 0.99609375]` output when it becomes available.

No redistributable recorded ECG window with reviewed expected peaks/model
inputs is checked into this repository. Do not use the analytic preprocessing
regression as evidence of clinical peak-detection performance.
...
- Confirm a phone decodes the exact transmitted signed ECG and status bytes;
  confirm real inference class, confidence, and end-of-window timestamp bytes.
...
- Run end-to-end acquisition, preprocessing, inference, and classification with
  reviewed ECG inputs; capture the reported Invoke and pipeline timing.
